...
local function SpectatorCount()
    ...
    if now >= nextSpecUpdate then
        ...
        local shared = Shared()
        specCount = shared and #shared.GetSpectators() or 0
    end
...
local function ChooseTeam(teamID)
    if not isnumber(teamID) then return end
...
local function CreateSpectatorButton(parent, x, y, width)
    ...
    button.DoClick = function() ChooseTeam(CSGOConfig.TeamSpectator) end
    return button
end
...
local function OpenTeamMenu()
    ...
    CreateTeamButton(frame, CSGOConfig.TeamCT, margin, 100, buttonWidth, "COUNTER-TERRORISTS", colors.ct)
    CreateTeamButton(frame, CSGOConfig.TeamT, margin + buttonWidth + gap, 100, buttonWidth, "TERRORISTS", colors.t)
