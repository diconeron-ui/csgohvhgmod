...
hook.Add("PlayerButtonDown", "CSGO.SpectatorInput", function(ply, button)
    if ply ~= LocalPlayer() then return end

    local action = actions[button]
    if not action then return end

    if not IsValid(ply) or ply:GetObserverMode() == OBS_MODE_NONE then return end
    if vgui.CursorVisible() or gui.IsGameUIVisible() then return end

    local now = RealTime()
    if now < nextInput then return end
    nextInput = now + 0.12

    net.Start("CSSpec_Input")
...
local function ModeName(mode)
    local shared = Shared()
    return shared and shared.GetModeName(mode) or "КАМЕРА"
end

local function TeamColor(teamID)
    if teamID == CSGOConfig.TeamCT then return colors.ct end
    if teamID == CSGOConfig.TeamT then return colors.t end
    return colors.muted
end
...
hook.Add("HUDPaint", "CSGO.SpectatorHUD", function()
    ...
    if mode ~= OBS_MODE_ROAMING and IsValid(target) and target:IsPlayer() then
        ...
        draw.SimpleTextOutlined(ModeName(mode), "CSSpecLabel", center, y + 70, colors.muted, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER, 1, colors.outline)
        ...
        draw.SimpleTextOutlined(ModeName(mode), "CSSpecTitle", center, y + 48, colors.text, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER, 1, colors.outline)
